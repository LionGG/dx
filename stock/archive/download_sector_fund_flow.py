...
def download_sector_fund_flow():
    ...
    for idx, row in df_list.iterrows():
        ...
        try:
            ...
            # 保存到数据库
            df[['date', 'sector_code', 'sector_name', 'main_net_inflow', 'main_net_inflow_pct',
...
def verify():
    ...
    cursor.execute('SELECT COUNT(DISTINCT sector_code) FROM sector_fund_flow')
    ...
    print("\n样本数据:")
    cursor.execute('''
        SELECT sector_name, date, main_net_inflow, main_net_inflow_pct
        FROM sector_fund_flow
        ORDER BY date DESC, main_net_inflow DESC
        LIMIT 5
    ''')
    for row in cursor.fetchall():
        print(f"  {row[0]} {row[1]}: 主力净流入{row[2]/1e8:.2f}亿 ({row[3]:.2f}%)")
